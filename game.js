...
if (
  ...
  isPlayerNearCustomerIcon(player, c)
) {
  if (c.state === "ordered") {
    receiveOrder(c);
    return;
  ...
  // ✅ 念のための保険：まだ注文されていない客には配膳させない
  if (c.state !== "waiting") return;

  if (carryingRamen?.iconIndex === c.orderIcon) {
  ...
  for (let spot of ramenSpots) {
    ...
    if (dx < tileSize && dy < tileSize) {
      const picked = tryPickupRamen();
if (picked) return; // ラーメンを取ったときだけ止める
    }
  }
  ...
  customerList.forEach(c => {
    ...
    if (c.state === "waiting" && carryingRamen?.iconIndex === c.orderIcon) {
      const order = activeOrders.find(o =>
        o.customerId === c.chairId && o.orderId === carryingRamen.orderId
      );
      if (!order) return;

      activeOrders = activeOrders.filter(o => o.orderId !== carryingRamen.orderId);
      c.state = "done";
      carryingRamen = null;
      playerImage.src = "img/player/order_misaki.png";
      score += 100;
