...
if (c.state === "waiting" && carryingRamen === c.orderIcon) {
  c.state = "done";
  carryingRamen = null;
  playerImage.src = "img/player/order_misaki.png";
  score += 100;
...
function handleZAction() {
  customerList.forEach(c => {
    ...
    // ✅ 注文アイコンに隣接していて、注文状態なら受け取る
    if (dx + dy <= tileSize + 10 && c.state === "ordered") {
      receiveOrder(c);
    }

    // ✅ 注文済みラーメンを渡せる場合
if (dx + dy <= tileSize + 10 && c.state === "waiting" && carryingRamen === c.orderIcon) {
  c.state = "done";
  carryingRamen = null;
  playerImage.src = "img/player/order_misaki.png";
  score += 100;
  ...
  tryPickupRamen(); // ← カウンター前のZでラーメン取得も対応
}
