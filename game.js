...
canvas.addEventListener("touchend", function (e) {
  ...
  const canvasX = touchX * canvas.width / canvas.clientWidth;
  const canvasY = touchY * canvas.height / canvas.clientHeight;
  ...
  // ★近くにお客様がいるかは常にチェック（移動できてもZアクションする）
  handleZAction();
});
...
function spawnCustomerLoop() {
  ...
  customerSpawnTimer = Math.max(3000, customerSpawnTimer - 1000);
